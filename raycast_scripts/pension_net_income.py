...
# @raycast.title Pension net income
...
# Documentation:
# @raycast.description Calculate the net income after tax for a given gross income Pension
...
# copy net income to clipboard. not print it
subprocess.run(["pbcopy"], input=str(tax.net_income), text=True)

# format 35000 to 35 000 NOK
formatted_net_income = "{:,}".format(tax.net_income).replace(",", " ")

print(f"Net income: {formatted_net_income} NOK")
